...
constraint ck_empleados_sexo check (sexo='H' or sexo='F')
);
...
constraint fk_historial_empleado_dni foreign key (empleado_dni) references empleados(dni),
constraint fk_historial_laboral_trabajo_cod foreign key ( trabajo_cod) references trabajos(trabajo_cod),
constraint fk_historial_dpto_cod foreign key (dpto_cod) references departamentos (dpto_cod),
