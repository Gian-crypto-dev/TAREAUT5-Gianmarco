...
constraint pk_patos_id_plato primary key (id_plato)
);
...
constraint fk_id_pedidos foreign key (id_pedido) references pedidos(id_pedido),
...
alter table pedidos add constraint fk_pedidos_id_cliente
foreign key (id_cliente) references clientes(id_cliente);

alter table clientes change c_postal codigo_postal numeric(5);

alter table platos add constraint ck_precio_plato check(precio between 1 and 100);
